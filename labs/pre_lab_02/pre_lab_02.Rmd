...
**Answer**
This is important because it preserves the original data set. It also allows for improved readability and you can perform operations on the filtered data set without affecting other analyses.
...
**Answer:**
The change in the expenditure_date column is that it went from month, day, year to year, month, day.
...
**Answer**
R is sorting the values of each month based upon the total expenditures from that month, then arranging them in descending order. It's clear that many of the 2022 months saw more expenditures.
...
**Answer**
This code is adding a total vote count and calculating the share of votes received by Wes Moore in each observation. 
...
**Answer**
The difference is that in the last task, we can now see the locations where Wes Moore performed the best because it's in descending order, whereas perviously it was in an unordered state.
...
**Answer**
It could be. Now it shows the locations where Wes Moore was considered unfavorably, which may speak more to the political affiliations of various locations in Maryland.
...
Chat GPT question: "What are some tidyverse functions similar to str_to_upper, and what do they do?"
Answer: str_to_lower, which converts all characters in a string to lowercase, and str_to_title, which converts the first letter of each to word to uppercase while keeping the rest lowercase.
...
**Answer**
The mutate statement is creating a new column called "state" in the "maryland_expenses" data set. It allows for each expense entry to be tagged with a corresponding state abbreviation.
...
**Answer**
Yes, this answer makes sense to me, as it details the list of states with the most total expenses to least amount of total expenses, listed in descending order. Therefore, it would make sense that Maryland is at the top.
...
1. Which Journalism courses are offered in 2025, and how many seats are available for each?

2. How many UMD courses were offered in Fall 2023, and how does that compare to the previous semester?

3. Which month had the highest total political expenditures, and how much was spent?
