...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer** This data set has four columns and 3,822 rows.
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** First, a month column is being added to the data set. The month(date, label=TRUE) function takes the month from the date column and makes it a three-letter abbreviation. The group function groups the data by the month column. The summarize creates a summary data set with one row per month, and arrange sorts the summarized data set in order based on total calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** To me, it just looks like a blank slate where a graph should ideally be.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** What I now see is a bar chart that has a y axis labeled as count and x axis labeled as month. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** I see a similar bar chart as last time, but now the reorder function has been added, so it's ordering months from least to most in terms of count.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** I see another bar chart, but now the coordinates have been flipped, so the month is on the y axis and count is on the x axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It looks very similar, except the them is being changed to theme_minimal, so it removes the gray background.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** Now, the theme has been changed even further, which now displays a blue-ish, and more stylish, background.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now, we've added a title, clear axis names, as well as a space for the source at the bottom.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** What I see are two columns, one of date and the other total calls. Basically, it shows how many calls were made on each specific date.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a line graph that looks extremely messy. It's basically following how many calls were made on each day, and the line goes from the number of calls on one date to the number of calls on another date, which helps to explain the messy nature of it.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** What I see is similar, but now, the date breaks are every single week, so we can see when some of the spikes in calls occur more specifically.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The only thing that was really changed here was was the text was titled at an angle to be able to read more clearly.
