...
It gives a warning advising me to run the "problems()" function to see what the problem is. When I run the function, it says "a double" was expected. The "guess_max" was too low.
...
No, there are no parsing errors this time.
...
"Vendor Name" is character, "Amount" is double and "Date" is character.
...
582 zip codes don't have five characters, which may be because they're abbreviated or because more than one zip code may start with those characters, explaining why there are longer zip codes now.
...
There are answers that have "NA" in the response, which could be incomplete data.
...
This code is arranging the loan data from least to greatest and saying who the grantee is.
...
The headers are now lowercase instead of uppercase.
...
The code is renaming the column name grantor to source and and mutating it to make the names in the column uppercase.
...
There would be 58 duplicates possible.
...
We're just taking a slice of the ZIP code and cutting down the number of characters in the ZIP code to the first five numbers on the left.
...
The top zip code was 21202. This zip code is Baltimore, which makes sense because it's the most populous place in Maryland and therefore would get the most money, logically.
